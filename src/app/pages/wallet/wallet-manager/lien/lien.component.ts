...
export class LienComponent implements OnInit {
  ...
  currentDeleteRecord: Lien;
  currentRecord: Lien;
  currentRecords: Array<Lien> = [];
  ...
  getRecords(){
    ...
    this.liens.recordRetrieve(query).then(res =>{
      if(res.success){
  ...
  queryRecord() {
    ...
    if (queryData.enabled !== '') {
      this.dirty === false ? this.dirty = true : this.searchMark = '&';
      searchQuery += `${this.searchMark}enabled=${queryData.enabled}`;
    }
    this.searchMark = '?'; this.dirty = false;

    this.liens.recordRetrieve(searchQuery).then(res =>{
      if (res.success) {
        this.currentRecords = res.payload;
