...
import { Schedules } from '../../../../providers';
...
export class ScheduleComponent implements OnInit {
  ...
  currentDeleteRecord: Schedule;
  currentRecord: Schedule;
  currentRecords: Array<Schedule> = [];
  ...
  getRecords(){
    let query = `?populate=countyFrom,countyTo,courier,stateFrom,stateTo`;
    this.schedules.recordRetrieve(query).then(res =>{
      if(res.success){
  ...
  queryRecord() {
    ...
    if (queryData.boardingDate !== '') {
      this.dirty === false ? this.dirty = true : this.searchMark = '&';
      searchQuery += `${this.searchMark}boardingDate=${queryData.boardingDate}`;
    }
    this.searchMark = '?'; this.dirty = false;

    this.schedules.recordRetrieve(searchQuery).then(res =>{
      if (res.success) {
        this.currentRecords = res.payload;
  ...
  openModal(record: Schedule): void {
    this.currentRecord = record;
