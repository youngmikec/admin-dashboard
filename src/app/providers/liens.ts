...
import { ApiResponse, Lien } from '../models';
...
export class Liens {
    ...
    constructor (private env: EnvService,
      ...
      for (const item of this.records) {
        this.records.push(new Lien(item));
      }
      this.recordRetrieve();
    }
    ...
    async recordRetrieve(queryString = ''): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/lien/${queryString}`;
    ...
    async recordCreate(data): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/lien`;
    ...
    async recordUpdate(record: Lien, payload): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/lien/${record.id}`;
