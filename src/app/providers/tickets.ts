...
export class Tickets {
    ...
    constructor (private env: EnvService,
      ...
      for (const item of this.records) {
        this.records.push(new Ticket(item));
      }
      this.recordRetrieve();
    }
    ...
    async recordRetrieve(queryString = ''): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/ticket/${queryString}`;
    ...
    async recordCreate(data): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/ticket`;
    ...
    async recordUpdate(record: Ticket, payload): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/ticket/operation/${record.id}`;
    ...
    async recordDelete(record: Ticket): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/ticket/${record.id}`;
