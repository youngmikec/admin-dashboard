...
export class Transactions {
    ...
    constructor (private env: EnvService,
      ...
      for (const item of this.records) {
        this.records.push(new Transaction(item));
      }
      this.recordRetrieve();
    }
    ...
    async recordRetrieve(queryString = ''): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transaction/${queryString}`;
    ...
    async recordCreate(data): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transaction`;
    ...
    async recordUpdate(record: Transaction, payload): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transaction/${record.id}`;
