...
export class Transfers {
    ...
    constructor (private env: EnvService,
      ...
      for (const item of this.records) {
        this.records.push(new Transfer(item));
      }
      this.recordRetrieve();
    }
    ...
    async recordRetrieve(queryString = ''): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transfer/${queryString}`;
    ...
    async recordCreate(data): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transfer`;
    ...
    async recordUpdate(record: Transfer, payload): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/transfer/${record.id}`;
