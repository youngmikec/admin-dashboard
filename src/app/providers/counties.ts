...
import { ApiResponse, County } from '../models';
...
export class Counties {
    ...
    records: Array<County> = [];

    constructor (private env: EnvService,
        private apiService: ApiService) {
      for (const item of this.records) {
        this.records.push(new County(item));
      }
      this.recordRetrieve();
    }
    ...
    async recordRetrieve(queryString = ''): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/county/${queryString}`;
    ...
    async recordCreate(data): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/county`;
    ...
    async recordUpdate(record: County, payload): Promise<ApiResponse> {
      const url = `${this.env.API_URL}/county/${record.id}`;
